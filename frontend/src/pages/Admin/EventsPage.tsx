...
export function EventsPage() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 text-[#1B4332]">
      ...
      {loading ? (
        <div className="bg-white border border-[#1B4332]/10 rounded-2xl overflow-hidden shadow-sm animate-pulse">
          ...
          {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 border-b border-[#1b4332]/5">
              <div className="w-16 h-10 bg-slate-200 rounded-lg shrink-0"></div>
              ...
              <div className="w-20 h-8 bg-slate-200 rounded-lg hidden sm:block"></div>
            </div>
          ))}
        </div>
      ) : events.length === 0 ? (
